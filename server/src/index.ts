...
const getTimeZoneDayIndex = (value: Date, timeZone: string) => {
    const { year, month, day } = getTimeZoneDateParts(value, timeZone);
    return Math.floor(Date.UTC(year, month - 1, day) / DAY_MS);
};

const getCalendarAnchorInTimeZone = (
    value: Date,
    timeZone = getDatabaseSessionTimeZone(),
) => {
    const { year, month, day } = getTimeZoneDateParts(value, timeZone);
    return new Date(Date.UTC(year, month - 1, day, 12, 0, 0, 0));
};

const parseBusinessDateValue = (value?: string | null) => {
...
const getRequestBusinessDate = (req: express.Request) =>
    parseBusinessDateValue(req.header(BUSINESS_DATE_HEADER)) ?? new Date();

const getRequestTransactionDate = (
    req: express.Request,
    timeZone = getDatabaseSessionTimeZone(),
) => {
...
const addCalendarDaysInTimeZone = (
    value: Date,
    daysToAdd: number,
    timeZone = getDatabaseSessionTimeZone(),
) => {
...
const getDayRangeInTimeZone = (value: Date, timeZone = getDatabaseSessionTimeZone()) => {
    const { year, month, day } = getTimeZoneDateParts(value, timeZone);
...
const getTimeZoneYmd = (value: Date, timeZone = getDatabaseSessionTimeZone()) => {
    const { year, month, day } = getTimeZoneDateParts(value, timeZone);
...
const getExistingPawnCountForDate = async (
    ...
    const rows = await executor
        ...
        .where(and(
            eq(cashTransactions.type, 'PAWN'),
            eq(items.type, GOLD_JEWELLERY_ITEM_TYPE),
            sql`date(${cashTransactions.date}) = ${ymd}`,
        ));
...
const resolveStoredLocation = <T extends {
    ...
    const storagePlacement = getStoragePlacementForDate(
        getCalendarAnchorInTimeZone(new Date(row.createdAt)),
    );
    return {
        ...row,
        physicalNumber: null,
...
const currentPrincipalByPawnId = async (
    executor: any = db,
    pawnIds?: number[],
) => {
    const conditions = [
        or(
            inArray(cashTransactions.type, PRINCIPAL_IN_TRANSACTION_TYPES as unknown as string[]),
            inArray(cashTransactions.type, PRINCIPAL_OUT_TRANSACTION_TYPES as unknown as string[]),
        )!,
    ];
    ...
    for (const tx of txs) {
        if (!tx.pawnId) continue;
        map.set(tx.pawnId, Number(tx.principal || 0));
    }
    return map;
};

const redemptionTotalsByPawnId = async (
    executor: any = db,
    pawnIds?: number[],
) => {
    const conditions = [
        inArray(cashTransactions.type, REDEMPTION_TRANSACTION_TYPES as unknown as string[]),
    ];
    if (pawnIds && pawnIds.length > 0) {
        conditions.push(inArray(cashTransactions.pawnFk, pawnIds));
    }
    ...
    const map = new Map<number, { redeemedAt?: Date; redeemedInterest: number; redeemedPrincipal: number }>();
...
const loadPawnStates = async (
    executor: any,
    pawnIds: number[],
) => {
    if (pawnIds.length === 0) return new Map<number, LoadedPawnState>();
...
const findDuplicatePawnIds = (pawnIds: number[]) => {
    const seen = new Set<number>();
    for (const pawnId of pawnIds) {
        if (seen.has(pawnId)) return pawnId;
        seen.add(pawnId);
    }
    return null;
};
...
const resolveEmployeeFk = async (requestedEmployeeId: unknown): Promise<number | null> => {
    const numericEmployeeId = Number(requestedEmployeeId);
    if (!Number.isFinite(numericEmployeeId) || numericEmployeeId <= 0) return null;
    ...
    try {
        ...
        return res.json({
            ...
            setup: {
                ...
                defaultAdminCredentials: userRows.some((row) =>
                    row.level === 'Admin' &&
                    row.userName === 'admin' &&
...
app.post('/setup/bootstrap', async (req, res) => {
    const adminUser =
        req.body?.adminUser && typeof req.body.adminUser === 'object'
            ? req.body.adminUser as Record<string, unknown>
            : {};
...
app.put('/users/:id', async (req, res) => {
    ...
    try {
        ...
        const updateData: { name: string; userName: string; level: string; password?: string } = { name, userName, level };
...
app.get('/pawns/:id', async (req, res) => {
    ...
    try {
        ...
        const daysDue = getDaysDueToToday(
            row.lastPaymentDate ? new Date(row.lastPaymentDate) : null,
...
app.post('/pawns', async (req, res) => {
    ...
    let customerPhoto: string | null = null;
    ...
    let customerFaceDescriptor: string | null = null;
    ...
    try {
        ...
        const created = await db.transaction(async (tx) => {
            ...
            const storageInfo: StorageInfo | null = useStorage
            ...
            if (Number.isFinite(requestedCustomerId) && requestedCustomerId > 0) {
                ...
                if (customerPhoto) updateFields.photo = customerPhoto;
                if (customerFaceDescriptor) updateFields.faceDescriptor = customerFaceDescriptor;
...
app.post('/pawns/batch/pay-interest', async (req, res) => {
    ...
    try {
        ...
        const results = normalizedTickets.map((ticket) => {
            ...
            const expectedAmount = calculateInterestAmountForPeriod(
                pawn.currentPrincipal,
                pawn.interestRate,
                baseDate,
                newLastPaymentDate,
            );
...
app.post('/pawns/:id/adjust', async (req, res) => {
    ...
    try {
        ...
        const interestDue = calculateInterestAmountForPeriod(
            currentLoan,
            Number(pawn.interestRate || 0),
            pawn.lastPaymentDate || pawn.createdAt || businessDate,
            businessDate,
        );
        if (interestDue > 0) {
            return res.status(400).json({
...
app.post('/pawns/batch/redeem', async (req, res) => {
    ...
    try {
        ...
        const results = normalizedTickets.map((ticket) => {
            ...
            const interestDue = calculateRedeemInterest(
                pawn.currentPrincipal,
                pawn.interestRate,
...
app.post('/pawns/batch/expire', async (req, res) => {
    const requestPawnIds: unknown[] = Array.isArray(req.body?.pawnIds)
        ? req.body.pawnIds
        : [];
    const pawnIds: number[] = requestPawnIds.length > 0
    ...
    try {
        ...
        await db.transaction(async (tx) => {
            await tx
                .update(items)
                .set({ status: 'EXPIRED' })
                .where(inArray(items.id, pawnRows.map((row) => row.itemId!).filter((value): value is number => Number.isFinite(value))));
        });
...
app.post('/pawns/:id/redeem', async (req, res) => {
    ...
    try {
        ...
        if (effectiveStatus !== 'PAWN') return res.status(400).json({ success: false, message: 'Pawn is not active' });
        if (!pawn.itemId) return res.status(400).json({ success: false, message: 'Item not found' });
...
app.put('/customers/:id', async (req, res) => {
    const id = Number(req.params.id);
    if (!Number.isFinite(id) || id <= 0) return res.status(400).json({ success: false, message: 'Invalid customer id' });
    ...
    try {
        ...
        let customerPhoto: string | null = (photo !== undefined) ? photo : null;
...
app.get('/customers/:id/pawns', async (req, res) => {
    ...
    if (!Number.isFinite(customerId) || customerId <= 0) return res.status(400).json({ success: false, message: 'Invalid customer id' });
    try {
        const rows = await db.select({
            id: pawns.id,
            customerId: customers.id,
...
app.get('/reports/daily-transactions', async (req, res) => {
    try {
        ...
        for (const tx of result) {
            if (tx.type === 'PAWN') pawnOut += tx.amount;
            else if (tx.type === 'REDEEM_BA' || tx.type === 'MINUS_AMOUNT') redeemIn += tx.amount;
            else if (
                tx.type === 'REDEEM_I' ||
...
app.get('/reports/top-customers', async (_req, res) => {
    try {
        const result = await db.select({
            ...
            name: customers.name,
            phone: customers.phone,
            pawnCount: sql<number>`count(distinct ${pawns.id})`,
            totalLoanAmount: sql<number>`
                coalesce(
                    sum(
                        case
            ...
            `,
            totalInterestAmount: sql<number>`
                coalesce(
                    sum(
                        case
...
app.put('/settings/app', async (req, res) => {
    const settingsPayload =
        req.body && typeof req.body.settings === 'object' && !Array.isArray(req.body.settings)
            ? req.body.settings
            : null;
    if (!settingsPayload) {
...
const start = async () => {
    try {
        ...
        throw new Error(
            `Unable to connect to Postgres using DATABASE_URL="${process.env.DATABASE_URL || ''}". ` +
            'Make sure PostgreSQL is running and reachable before launching the app.',
            { cause: error },
        );
    }
