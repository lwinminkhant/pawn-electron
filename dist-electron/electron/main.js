...
const writeRuntimeConfig = async (config) => {
...
const normalizeApiStartupMessage = (message) => {
...
const probeApiHealth = async (apiPort) => {
    try {
...
const waitForApiHealth = async (apiPort, timeoutMs = 12000) => {
...
const getApiEntryPath = () => app.isPackaged
...
const startManagedApi = async (config) => {
...
ipcMain.handle('desktop-setup:save-runtime-config', async (_event, payload) => {
