...
export const isFaceApiAvailable = async () => {
    return ensureDeps();
};
export const initFaceModels = async () => {
    ...
    if (!ok)
        throw new Error('Face detection dependencies not installed – unavailable in this environment');
...
export const detectFaceDescriptorFromBase64 = async (base64Image) => {
    if (!modelsLoaded) {
    ...
    return new Promise((resolve, reject) => {
        ...
        img.onerror = (err) => reject(new Error('Failed to load base64 as Image: ' + err));
        const dataUrl = base64Image.startsWith('data:image/') ? base64Image : `data:image/jpeg;base64,${base64Image}`;
        img.src = dataUrl;
    });
};
export const compareDescriptors = (probe, stored) => {
    return faceapi.euclideanDistance(Array.from(probe), Array.from(stored));
};
export const jsonToDescriptor = (json) => new Float32Array(JSON.parse(json));
export const descriptorToJson = (descriptor) => JSON.stringify(Array.from(descriptor));
